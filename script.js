  ...
  setTimeout(function(){
    ...
    setTimeout(function(){
      ...
      function tick(now){
        ...
        // Easing: karakter settle dari kiri ke kanan
        ...
        if(el.classList.contains('glitch')) el.setAttribute('data-text', result);
        if(p<1){
          rafId=requestAnimationFrame(tick);
        } else {
          el.textContent=finalText;
          if(el.classList.contains('glitch')) el.setAttribute('data-text', finalText);
        }
      }
      rafId=requestAnimationFrame(tick);
    }, 130);
...
function applyLanguage(lang){
  ...
  // Kumpulkan semua elemen yang teksnya berubah, lalu scramble dengan stagger
  var delay=0;
  document.querySelectorAll('[data-i18n]').forEach(function(el){
    const k=el.getAttribute('data-i18n');
    if(!T[lang]||!T[lang][k]) return;
    const newText=T[lang][k];
    if(el.textContent.trim()===newText) return; // skip kalau sama
    scrambleText(el, newText, 400, delay);
    delay+=20; // stagger antar elemen
  });

  // Re-render dynamic content setelah semua animasi selesai
  var totalDur=delay+550;
  setTimeout(function(){ renderDynamicContent(); }, totalDur);
}
...
const productsData = [
  ...
  { id:'aimbot', icon:'fa-crown', nameKey:'premium_name', price:10000, popular:false, featuresKeys:['premium_feat1','premium_feat2','premium_feat3','premium_feat4','premium_feat5'], dataProduct:'AIM FORCE' }
];
...
function renderProductCards(){
  ...
  productsData.forEach(prod=>{
    ...
    prod.featuresKeys.forEach((key,idx)=>{
      const li=document.createElement('li');
      const isLastPcNote = idx === prod.featuresKeys.length-1 && (key==='basic_feat5'||key==='premium_feat5');
      if(isLastPcNote){
        li.style.cssText='margin-top:10px;padding-top:10px;border-top:1px solid var(--border);opacity:.75;font-size:.82em;';
        li.innerHTML=`<i class="fas fa-desktop" style="color:var(--primary-light);"></i><span data-i18n="${key}" style="font-style:italic;">${T[currentLang]?.[key]||''}</span>`;
      } else {
        li.innerHTML=`<i class="fas fa-check-circle"></i><span data-i18n="${key}">${T[currentLang]?.[key]||''}</span>`;
      }
      ul.appendChild(li);
    });
    card.appendChild(ul);
...
function renderTestimonials(){
  ...
  testimonialsData.forEach(t=>{
    ...
    card.setAttribute('data-i18n-html', t.textKey);
    card.innerHTML=T[currentLang]?.[t.textKey]||T['id'][t.textKey]||'';
...
function showDiscordPopup(onContinue){
  const lang=currentLang;
  const t=T[lang]||T['id'];
  let popup=document.getElementById('discordPopup');
  if(!popup){
    popup=document.createElement('div');
    popup.id='discordPopup';
    popup.style.cssText='position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.75);backdrop-filter:blur(6px);opacity:0;transition:opacity .25s;';
    popup.innerHTML=`
      <div id="discordPopupBox" style="background:var(--surface-glass);border:1px solid var(--border);border-radius:24px;padding:36px 32px;max-width:420px;width:90%;text-align:center;transform:scale(.92) translateY(20px);transition:transform .3s cubic-bezier(.23,1,.32,1),opacity .3s;opacity:0;backdrop-filter:blur(20px);box-shadow:0 20px 60px rgba(0,0,0,.5);">
        <div style="font-size:3rem;margin-bottom:12px;"><i class="fab fa-discord" style="color:#5865F2;"></i></div>
        <h3 id="dpTitle" style="font-size:1.3rem;font-weight:800;margin-bottom:14px;background:linear-gradient(135deg,var(--primary-light),var(--primary));-webkit-background-clip:text;background-clip:text;color:transparent;"></h3>
        <p id="dpMsg" style="font-size:0.97rem;color:var(--text-secondary);line-height:1.6;margin-bottom:28px;"></p>
        <div style="display:flex;flex-direction:column;gap:12px;">
          <button id="dpJoin" style="background:linear-gradient(135deg,#5865F2,#7289da);color:#fff;border:none;border-radius:14px;padding:14px 24px;font-size:1rem;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;transition:transform .15s,box-shadow .15s;">
            <i class="fab fa-discord"></i> <span id="dpJoinTxt"></span>
          </button>
          <button id="dpSkip" style="background:transparent;color:var(--text-secondary);border:1px solid var(--border);border-radius:14px;padding:12px 24px;font-size:0.9rem;font-weight:600;cursor:pointer;transition:border-color .15s,color .15s;">
            <span id="dpSkipTxt"></span>
          </button>
        </div>
      </div>
    `;
    // CSS highlight untuk <mark>
    const st=document.createElement('style');
    st.textContent='#discordPopup mark{background:none;color:var(--primary-light);font-weight:800;text-decoration:underline;text-underline-offset:3px;}#dpJoin:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(88,101,242,.5);}#dpSkip:hover{border-color:var(--primary-light);color:var(--primary-light);}';
    document.head.appendChild(st);
    document.body.appendChild(popup);
  }
  // Update teks sesuai bahasa
  document.getElementById('dpTitle').textContent=t.discord_popup_title||'Hemat Lebih Banyak!';
  document.getElementById('dpMsg').innerHTML=t.discord_popup_msg||'Dapatkan harga lebih <mark>murah</mark> dan info lebih detail dari server <mark>Discord</mark> kami. Harap join Discord!';
  document.getElementById('dpJoinTxt').textContent=t.discord_popup_join||'Join Discord';
  document.getElementById('dpSkipTxt').textContent=t.discord_popup_skip||'Lewati';

  popup.style.display='flex';
  void popup.offsetWidth;
  popup.style.opacity='1';
  const box=document.getElementById('discordPopupBox');
  box.style.transform='scale(1) translateY(0)';
  box.style.opacity='1';

  function closePopup(){ 
    popup.style.opacity='0';
    box.style.transform='scale(.92) translateY(20px)';
    box.style.opacity='0';
    setTimeout(()=>{ popup.style.display='none'; },280);
  }

  document.getElementById('dpJoin').onclick=function(){
    closePopup();
    // Buka discord lalu lanjut ke payment
    window.open('https://discord.gg/sJ3M77ChQH','_blank','noopener,noreferrer');
    setTimeout(()=>onContinue(),400);
  };
  document.getElementById('dpSkip').onclick=function(){
    closePopup();
    setTimeout(()=>onContinue(),280);
  };
  // Click luar kotak = skip
  popup.onclick=function(e){ if(e.target===popup){ document.getElementById('dpSkip').onclick(); } };
}
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  (function(){
    ...
    function updateBadgeText(){
      const txt=T[currentLang]?.server_online||'SERVER ONLINE';
      badge.innerHTML='<span style="width:7px;height:7px;margin:0 5px 0 0;display:inline-block;vertical-align:middle;background:#22c55e;border-radius:50%;position:relative;box-shadow:0 0 6px #22c55e;"></span>'+txt;
    }
    updateBadgeText();
    document.body.appendChild(badge);
    // Patch applyLanguage agar badge ikut update
    const _origApply=window._applyLangPatched;
    const origNode=badge;
    const obs=new MutationObserver(()=>updateBadgeText());
    // Update badge setiap kali lang berubah via currentLang watcher
    const applyOrig=window.applyLanguage;
    // Hook: re-update badge text after every language switch
    // (karena applyLanguage sudah didefinisikan sebelumnya, kita patch via interval ringan)
    let lastLang=currentLang;
    setInterval(()=>{
      if(currentLang!==lastLang){lastLang=currentLang;updateBadgeText();}
    },300);
  })();
